...
echo
echo "[setup] Optional SVF backend (LLVM 21 only at present):"
echo "          bash scripts/build_svf.sh ${LLVM_MAJOR}"
echo "          make -C analyzer SVF=1 BUILD=build-svf LLVM_CONFIG=${LLVM_CONFIG}"
